...
def looks_like_youtube_key(key: str | None) -> bool:
    if not key or not YT_KEY.fullmatch(key):
        return False
    # TMDB page noise often yields CSS fragments like "mobile-web-"
    ...
    for url in urls:
        try:
            ...
            if r.status_code != 200:
                continue
            # Prefer official trailer labels
...
def main():
    ...
    # scrub previously poisoned keys
    for row in rows:
    ...
    for i, row in enumerate(missing, 1):
        key = find_trailer(
            row["id"], row.get("title") or row.get("query") or "", row.get("year")
        )
        if key:
            row["trailer_key"] = key
            filled += 1
